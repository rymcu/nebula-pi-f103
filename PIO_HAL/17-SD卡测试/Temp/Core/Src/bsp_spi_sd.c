...
uint8_t buff[4];
static void SD_CS_Enable(void)  { HAL_GPIO_WritePin(GPIOC, GPIO_PIN_4, GPIO_PIN_RESET);HAL_SPI_Transmit(&hspi1, (uint8_t[]){0xFF}, 1, 100);HAL_SPI_Receive(&hspi1, buff, 1, 100);}
static void SD_CS_Disable(void) { HAL_GPIO_WritePin(GPIOC, GPIO_PIN_4,   GPIO_PIN_SET);HAL_SPI_Transmit(&hspi1, (uint8_t[]){0xFF}, 1, 100); }
...
uint8_t SD_Init(void) {
    ...
    for(uint8_t i=0; i<9; i++) HAL_SPI_Transmit(&hspi1, (uint8_t[]){0xFF}, 1, 100);
    SD_CS_Enable();
    // 发送CMD0（带超时）
    timeout = SD_INIT_TIMEOUT;
    do {
        response = SD_SendCommand(SD_CMD0, 0, 0x95);
        if(--timeout == 0) return 0xF0; // 错误码0xFF表示CMD0无响应
    } while(response != 0x01);

    // 发送CMD8检测卡类型
    response = SD_SendCommand(SD_CMD8, 0x1AA, 0x87);
    //return response;
    if(response == 0x01) {  // SDC V2+
        HAL_SPI_Receive(&hspi1, ocr, 4, 100);
        if(ocr[3] != 0xAA) return 0xFE; // 错误码0xFE表示电压不匹配
        //return ocr[3];
        // 发送ACMD41初始化（带超时）
        timeout = SD_INIT_TIMEOUT;
        do {
            ...
            if(--timeout == 0) return 0xFD; // 错误码0xFD表示初始化超时
        } while(response != 0x00);
    ...
    else if(response == 0x05) {  // SDC V1
        timeout = SD_INIT_TIMEOUT;
        do {
            response = SD_SendCommand(SD_CMD1, 0, 0xF9);
            if(--timeout == 0) return 0xFC; // 错误码0xFC表示V1卡初始化失败
        } while(response != 0x00);
        sd_type = SD_TYPE_V1;
    }
    else {
...
uint8_t SD_WriteBlock(uint32_t block_addr, uint8_t *buffer) {
    ...
    if(SD_SendCommand(SD_CMD24, block_addr, 0xFF) != 0x00) return 1;
...
uint32_t SD_GetCapacity(void) {
    ...
    if(SD_SendCommand(SD_CMD9, 0, 0xFF) != 0x00) return 0;
